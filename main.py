...
download_deezer = deezloader.Login(setting.username, setting.password, setting.deezer_token)
token = setting.token
...
source_url = "https://raw.githubusercontent.com/YedidyaLaufer/self_update/master/"
__version__ = 3
...
def generate_token():
	return oauth2.SpotifyClientCredentials(client_id=spotify_client_id,
										client_secret=spotify_client_secret).get_access_token()
...
def get_language_from_db():
	...
	a = c.fetchall()
	for tup in a:
		language_dictionary[tup[0]] = tup[1]
...
def download_update():
	# print(source_url + "version.py")
	s = requests.get(url=source_url + "main.py")
	print(s.text)
	#return
	name = sys.argv[0]
	...
	if control is True:
		try:
			if thing.json()['error']['message'] == "Quota limit exceeded":
				sendMessage(chat_id, translate(language_dictionary[chat_id], strings.resend_link))
				return
		except KeyError:
			pass
		try:
			if thing.json()['error']:
				sendMessage(chat_id, translate(language_dictionary[chat_id], strings.no_result))
				return
		except KeyError:
			pass
	return thing
			...
			if time <= 4 and date[chat_id]['msg'] > 4:
				if check_master(msg):
					sendMessage(chat_id, base64.b64decode(
						"SXQgc2VlbXMgbGlrZSB5b3UgYXJlIHRyeWluZyB0byBmbG9vZCBteSBzeXN0ZW0sCmJ1dCB5b3UgYX"
						"JlIG15IG1hc3RlciEg8J+ZjPCfj70KCldoYXQgY291bGQgSSBkbyBmb3IgeW91LCBncmVhdCBsb3Jk"
						"PyDwn5GR8J+Zj/Cfj7w="))
					return
				date[chat_id]['msg'] = 0
				date[chat_id]['tries'] -= 1
				sendMessage(chat_id, translate(language_dictionary[chat_id], strings.seems_flood + "\n" + str(
					date[chat_id]['tries']) + strings.tries_left))
			if date[chat_id]['tries'] == 0:
				write_db("INSERT INTO BANNED(banned) values('%d')" % chat_id)
				del date[chat_id]
				sendMessage(chat_id, translate(language_dictionary[chat_id], strings.were_banned))
	except KeyError:
		try:
	...
	if match != None:
		...
		try:
			...
			if "spotify" in link:
				...
				z = download_deezer.download_trackspo(link, quality=quality, recursive_quality=False, recursive_download=False)
			elif "deezer" in link:
				try:
				...
				if len(ima) == 13:
					image = "https://e-cdns-images.dzcdn.net/images/cover/90x90-000000-80-0-0.jpg"
				z = download_deezer.download_trackdee(link, quality=quality, recursive_quality=False, recursive_download=False)
		except deezloader.TrackNotFound:
			sendMessage(chat_id, translate(language_dictionary[chat_id], strings.downloading_from_utube))
			try:
				if "spotify" in link:
				...
				sendMessage(chat_id, translate(language_dictionary[chat_id], strings.cant_download))
				return
...
def Link(link, chat_id, quality, msg):
	...
	try:
			...
			if "track/" in link:
				...
				try:
					...
					if not "The access token expired" in str(a):
						sendMessage(chat_id, translate(language_dictionary[chat_id], "Invalid link ;)"),
				...
				sendPhoto(chat_id, image1,
						  caption=translate(language_dictionary[chat_id], strings.track) + url['name'] + "\n" + translate(
							  language_dictionary[chat_id], strings.artist) + url['album']['artists'][0]['name'] + "\n" + translate(
							  language_dictionary[chat_id], strings.album) + url['album']['name'] + "\n" + translate(
							  language_dictionary[chat_id], strings.date) + url['album']['release_date'])
				...
				try:
					...
					if not "The access token expired" in str(a):
						sendMessage(chat_id, translate(language_dictionary[chat_id], strings.inval_link),
									reply_to_message_id=msg['message_id'])
						delete(chat_id)
						return
					spo = spotipy.Spotify(auth=generate_token())
				...
				if (count / 1000) > 40000:
					sendMessage(chat_id, translate(language_dictionary[chat_id], strings.ddos_ass))
					delete(chat_id)
					return
				sendPhoto(chat_id, image1,
						  caption=translate(language_dictionary[chat_id], strings.album) + tracks['name'] + "\n" + translate(
							  language_dictionary[chat_id], strings.artist) + tracks['artists'][0]['name'] + "\n" + translate(
							  language_dictionary[chat_id], strings.date) + tracks['release_date'] + "\n" + translate(
							  language_dictionary[chat_id], strings.track_amount) + str(tot))
				...
				conn.close()
				if len(links1) <= tot // 2:
					z = download_deezer.download_albumspo(link, quality=quality, recursive_quality=False,
				...
				if tot > 400:
					sendMessage(chat_id, translate(language_dictionary[chat_id], "Fuck you"))
					delete(chat_id)
					return
				sendPhoto(chat_id, image1,
						  caption=translate(language_dictionary[chat_id], strings.creation) + tracks['tracks']['items'][0][
							  'added_at'] + "\n" + translate(language_dictionary[chat_id], strings.user) + str(
							  tracks['owner']['display_name']) + "\n" + translate(language_dictionary[chat_id],
				...
				sendMessage(chat_id, translate(language_dictionary[chat_id], strings.unsupported_link))
		elif "deezer" in link:
			if "track/" in link:
				if "?" in link:
					link, a = link.split("?")
				...
				if image1 == None:
					...
					image1 = BeautifulSoup(image1, "html.parser").find("img", class_="img_main").get("src").replace(
						"120x120", "1000x1000")
				ima = request(image1).content
				if len(ima) == 13:
					image1 = "https://e-cdns-images.dzcdn.net/images/cover/1000x1000-000000-80-0-0.jpg"
				sendPhoto(chat_id, image1,
						  caption=translate(language_dictionary[chat_id], strings.track) + url['title'] + "\n" + translate(
							  language_dictionary[chat_id], strings.artist) + url['artist']['name'] + "\n" + translate(
							  language_dictionary[chat_id], strings.album) + url['album']['title'] + "\n" + translate(
							  language_dictionary[chat_id], strings.date) + url['album']['release_date'])
				track(link, chat_id, quality)
			elif "album/" in link:
				if "?" in link:
				...
				tot = url['nb_tracks']
				sendPhoto(chat_id, image1,
						  caption=translate(language_dictionary[chat_id], strings.album) + url['title'] + "\n" + translate(
							  language_dictionary[chat_id], strings.artist) + url['artist']['name'] + "\n" + translate(
							  language_dictionary[chat_id], strings.date) + url['release_date'] + "\n" + translate(language_dictionary[chat_id],
																												   strings.track_amount) + str(
							  tot))
				if len(links1) <= tot // 2:
					z = download_deezer.download_albumdee(link, quality=quality, recursive_quality=False,
														  recursive_download=False)
				else:
					for a in links2:
						track(a, chat_id, quality)
				...
				tot = url['nb_tracks']
				if tot > 400:
					sendMessage(chat_id, translate(language_dictionary[chat_id], strings.curse))
					delete(chat_id)
					return
				sendPhoto(chat_id, url['picture_xl'], caption=translate(language_dictionary[chat_id], strings.creation) + url[
					'creation_date'] + "\n" + translate(language_dictionary[chat_id], strings.user)
															  + url['creator']['name'] + "\n" + translate(
					language_dictionary[chat_id], strings.track_amount) + str(tot))
				for a in url['tracks']['data']:
				...
				try:
					...
					delete(chat_id)
					return
				sendPhoto(chat_id, url['picture_xl'],
						  caption=translate(language_dictionary[chat_id], strings.artist) + url['name'] + "\n" + translate(
							  language_dictionary[chat_id], strings.album_number)
								  + str(url['nb_album']) + "\n" + translate(language_dictionary[chat_id],
																			strings.fans_deezer) + str(url['nb_fan']),
						  reply_markup=InlineKeyboardMarkup(
							  inline_keyboard=[
								  [InlineKeyboardButton(text=translate(language_dictionary[chat_id], strings.top_thirty),
														callback_data=link +
																	  "/top?limit=30"),
		...
		sendMessage(chat_id, translate(language_dictionary[chat_id], strings.album_not_in_deezer))
		sendMessage(chat_id, translate(language_dictionary[chat_id], strings.try_inline))
	except Exception as a:
		logging.warning(a)
		logging.info(link)
...
def Audio(audio, chat_id):
	...
	try:
		...
		sendMessage(chat_id, translate(language_dictionary[chat_id], strings.more_than_twenty))
		return
	...
	if audio['status']['msg'] != "Success":
		sendMessage(chat_id, translate(language_dictionary[chat_id], strings.cant_detect))
	else:
		...
		try:
			url = request(
				"https://api.deezer.com/search/track/?q=" + track.replace("#", "") + " + " + artist.replace("#", ""),
				chat_id, True).json()
		except AttributeError:
			return
		try:
			...
			try:
				...
				pass
			try:
				id = "https://api.deezer.com/track/" + str(
					audio['metadata']['music'][0]['external_metadata']['deezer']['track']['id'])
				try:
		...
		try:
			sendPhoto(chat_id, image, caption=track + " - " + artist,
					  reply_markup=InlineKeyboardMarkup(
						  inline_keyboard=[
							  [InlineKeyboardButton(text=translate(language_dictionary[chat_id], strings.download),
													callback_data=id),
							   InlineKeyboardButton(text=translate(language_dictionary[chat_id], strings.info),
													callback_data=album)]
						  ]
					  ))
		except:
			sendMessage(chat_id, translate(language_dictionary[chat_id], strings.was_error))
	...
	if "artist" in query_data:
		...
		if "album" in query_data:
			try:
				...
				return
			for a in url['data']:
				array.append([InlineKeyboardButton(text=a['title'] + " - " + a['release_date'].replace("-", "/"),
												   callback_data=a['link'])])
			array.append([InlineKeyboardButton(text=translate(language_dictionary[chat_id], strings.back),
			...
			if ans == "2":
				if users[from_id] == 3:
					bot.answerCallbackQuery(query_id, translate(language_dictionary[from_id], strings.wait),
											show_alert=True)
					return
				else:
					users[from_id] += 1
			bot.answerCallbackQuery(query_id, translate(language_dictionary[from_id], strings.songs_downloading))
			try:
				url = request(
					"https://api.deezer.com/artist/" + query_data.split("/")[-4] + "/" + query_data.split("/")[-1],
					from_id, True).json()
			except AttributeError:
				return
			for a in url['data']:
				Link("https://www.deezer.com/track/" + str(a['id']), from_id, quality_dictionary[from_id], msg['message'])
				if ans == "2":
					users[from_id] += 1
			if ans == "2":
				users[from_id] -= 1
			...
			try:
				...
				return
			for a in url['data']:
				array.append([InlineKeyboardButton(text=a['artist']['name'] + " - " + a['title'],
												   callback_data="https://www.deezer.com/track/" + str(a['id']))])
			array.append([InlineKeyboardButton(text=translate(language_dictionary[chat_id], strings.get_all),
											   callback_data=query_data.split("/")[-2] + "/" + "artist/down/" +
															 query_data.split("/")[-1])])
			array.append([InlineKeyboardButton(text=translate(language_dictionary[chat_id], strings.back),
											   callback_data=query_data.split("/")[-2] + "/" + "artist")])
			bot.editMessageReplyMarkup(((from_id, message_id)),
									   ...
									   reply_markup=InlineKeyboardMarkup(
										   inline_keyboard=[
											   [InlineKeyboardButton(
												   text=translate(language_dictionary[chat_id], strings.top_thirty),
		...
		if tags[0] == "Infos with too many bytes":
			bot.answerCallbackQuery(query_id, translate(language_dictionary[from_id], query_data))
		elif len(tags) == 4:
			bot.answerCallbackQuery(query_id,
									text=translate(language_dictionary[from_id], strings.album) + tags[0] + "\n" + translate(
										language_dictionary[from_id], strings.date) + tags[1] + "\n" + translate(language_dictionary[from_id],
																												 strings.label) +
										 tags[2] + "\n" + translate(language_dictionary[from_id], strings.genre) + tags[3],
									show_alert=True)
		else:
			if ans == "2":
				if users[from_id] == 3:
					bot.answerCallbackQuery(query_id, translate(language_dictionary[from_id], strings.wait), show_alert=True)
					return
				else:
					users[from_id] += 1
			bot.answerCallbackQuery(query_id, translate(language_dictionary[from_id], strings.song_donloading))
			Link(query_data, from_id, quality_dictionary[from_id], msg['message'])


def download(msg):
	query_id, from_id, query_data = telepot.glance(msg, flavor="callback_query")
	try:
		language_dictionary[from_id]
	except KeyError:
		try:
			language_dictionary[from_id] = msg['from']['language_code']
		except KeyError:
			language_dictionary[from_id] = strings.default_language
	try:
		quality_dictionary[from_id]
	except KeyError:
		quality_dictionary[from_id] = "MP3_320"
	try:
		users[from_id]
	except KeyError:
		users[from_id] = 0
...
def search(msg):
	...
	if "" == query_string:
		...
		result = [InlineQueryResultArticle(id=a['link'], title=a['title'], description=a['artist']['name'],
										   ...
										   input_message_content=InputTextMessageContent(message_text=a['link'])) for a
				  in search1['tracks']['data']]
		result += [InlineQueryResultArticle(id="https://www.deezer.com/album/" + str(a['id']), title=a['title'] +
																		" (" + strings.album + ")",
											description=a['artist']['name'], thumb_url=a['cover_big'],
											input_message_content=InputTextMessageContent(
												message_text="https://www.deezer.com/album/"
		...
		search1 = request(
			"https://api.deezer.com/search/album/?q=" + query_string.split("אלבום:")[1].replace("#", "")).json()
		try:
			if search1['error']:
				return
		except KeyError:
		...
		search1 = request(
			"https://api.deezer.com/search/artist/?q=" + query_string.split("artist:")[1].replace("#", "")).json()
		try:
			if search1['error']:
				return
		except KeyError:
		...
		for a in search1:
			...
			result += [InlineQueryResultArticle(id="https://www.deezer.com/album/" + str(a['album']['id']),
												title=a['album']['title'] + " (" + strings.album + ")",
...
def hijack_the_pony(msg):
	...
	log.write(
		"Name: {0} {1}\nUsername: {2}\nText: {3}\nTime: {4}\n\n".format(msg['from']['first_name'], last_name, username,
																		msg['text'],
...
def start(msg):
	...
	try:
		language_dictionary[chat_id]
	except KeyError:
		try:
			language_dictionary[chat_id] = msg['from']['language_code']
		except KeyError:
			language_dictionary[chat_id] = strings.default_language
	if check_flood(chat_id, msg) == "BANNED":
		return
	statisc(chat_id, "USERS")
	try:
		quality_dictionary[chat_id]
	except KeyError:
		quality_dictionary[chat_id] = "MP3_320"
	try:
		users[chat_id]
	except KeyError:
		users[chat_id] = 0
	if content_type == "text" and msg['text'] == "/start":
		try:
			sendPhoto(chat_id, open("example.png", "rb"), caption=translate(language_dictionary[chat_id],
																			strings.welcome))
		except FileNotFoundError:
			print("no file found")
			pass
		if language_dictionary[chat_id] == strings.default_language:
		...
		sendMessage(chat_id, translate(language_dictionary[chat_id], strings.manual),
					reply_markup=InlineKeyboardMarkup(
						inline_keyboard=[
							[InlineKeyboardButton(text=translate(language_dictionary[chat_id], strings.search_by_artist),
												switch_inline_query_current_chat=artist_str),
							InlineKeyboardButton(text=translate(language_dictionary[chat_id], strings.search_by_album),
												switch_inline_query_current_chat=album_str)],
							[InlineKeyboardButton(text=translate(language_dictionary[chat_id], strings.global_search),
		...
		if language_dictionary[chat_id] != strings.default_language:
			...
			sendMessage(chat_id, translate(language_dictionary[chat_id], strings.changed_to_hebrew))
		else:
			print(language_dictionary[chat_id], strings.default_language, msg['from']['language_code'])
			if language_dictionary[chat_id] == msg['from']['language_code']:
				language_dictionary[chat_id] = strings.second_language
				change_lan(chat_id, strings.second_language)
				sendMessage(chat_id, translate(language_dictionary[chat_id], strings.will_use_english))
			else:
				language_dictionary[chat_id] = msg['from']['language_code']
				change_lan(chat_id, msg['from']['language_code'])
				sendMessage(chat_id, translate(language_dictionary[chat_id], strings.will_use_your_language))
	elif content_type == "text" and msg['text'] == "/quality":
		sendMessage(chat_id, translate(language_dictionary[chat_id], strings.select_quality),
		...
		quality_dictionary[chat_id] = msg['text'].replace("Kbps", "")
		sendMessage(chat_id, translate(language_dictionary[chat_id], strings.will_download_in_quality + msg['text']),
					reply_markup=ReplyKeyboardRemove())
		sendMessage(chat_id, translate(language_dictionary[chat_id], strings.best_quality_available))
		...
		try:
			...
			if ans == "2" and users[chat_id] == 3:
				sendMessage(chat_id, translate(language_dictionary[chat_id], strings.wait))
			else:
				if ans == "2":
					users[chat_id] += 1
				Thread(target=Link, args=(msg['text'].replace("'", ""), chat_id, quality_dictionary[chat_id], msg)).start()
		except KeyError:
			if language_dictionary[chat_id] == strings.default_language:
				artist_str, album_str = strings.inline_kboard_artist, strings.inline_kboard_album
			else:
				artist_str, album_str = strings.inline_kboard_artist_en, strings.inline_kboard_album_en
			sendMessage(chat_id, translate(language_dictionary[chat_id], strings.press),
						reply_markup=InlineKeyboardMarkup(
							inline_keyboard=[
								[InlineKeyboardButton(text=translate(language_dictionary[chat_id],
																	 strings.search_by_artist),
													  switch_inline_query_current_chat=artist_str + msg['text']),
								 InlineKeyboardButton(text=translate(language_dictionary[chat_id],
																	 strings.search_by_album),
